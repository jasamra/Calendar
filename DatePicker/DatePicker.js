...
let currentTimeStamp = Date.now() - (Date.now() % oneDay) + (new Date().getTimezoneOffset() * 1000 * 60);
...
export default class DatePicker extends Component {
    ...
    constructor() {
        ...
        this.state = {
            ...
            monthInfo: this.getMonthInfo(year, month)
        }
    }
    ...
    getDays (args) {
        ...
        let prevMonthNumDays = this.getNumberOfDays(prevYear, prevMonth);
        let dated = (date < 0 ? prevMonthNumDays+date : date % args.numberOfDays) + 1;
        let month = date < 0 ? -1 : date >= args.numberOfDays ? 1 : 0;
        let timestamp = new Date(args.year, args.month, dated).getTime();
    ...
    isCurrentDay (day) {
        return day.timestamp === currentTimeStamp;
    }
    ...
    updateDateFromInput () {
        ...
        if(dateData !== null) { 
            ...
            this.setState({ 
                ...
                monthInfo: this.getMonthInfo(dateData.year, dateData.month-1)
            })
        }
    }

    setDateToInput (timestamp) {
        let dateString = this.getTimestamp(timestamp);
    ...
    renderCalendar() {
        let days = this.state.monthInfo.map((day, index)=> {
            return (
                <div className={'c-day-container ' + (day.month !== 0 ? ' disabled' : '') + 
                    (this.isCurrentDay(day) ? ' highlight' : '') + (this.isSelected(day) ? ' highlight-green' : '')} key={index}>
